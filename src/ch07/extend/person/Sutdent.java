...
/*
     여러 개의 일반적인 속성(attribute)과 기능(operation)이 있더라.
...
public class Sutdent extends Person {
    ...
    //Student 는 name, age 필드, introduce() 메서드를 그냥 사용할 수 있습니다.

    // 빈 생성자는 자동으로 상속받음
    // 하지만 매개변수가 있는 생성자는 상속할 때 별도로 선언해줘야함

public  Sutdent(String name, int age, String studentId) {

    // 부모의 생성자를 호출
    // super(...);
    // 부모 객체도 실제로 생성이 됨
    super(name, age); //부모 메서드의 매개변수로 호출
...
@Override           // overloading : 메서드 시그니처(메서드명,메서드매개변수개수타입
//                                        메서드명은 동일하고,메서드매개변수개수타입만 바꿔서 정의
//               override : (재정의한다)
//               부모클래스의 메서드를 재정의 메서드 시그니처 동일하고 구현부만 변경함
